...
void loop() {
  ...
  if (I2C_begin(0x13) == 0) { // Kiểm tra nếu master gửi dữ liệu đến (Master writes - Slave reads)
    receivedData = I2C_readByte();
    Serial.print("Received data: ");
    Serial.println(receivedData);
  }
  else if (I2C_begin(0x13) == 1) { // Kiểm tra nếu master yêu cầu đọc dữ liệu (Master reads - Slave writes)
